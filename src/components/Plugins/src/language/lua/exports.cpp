...
void SetupLuaExports(luacpp::LuaState *state, Plugin *plugin)
{
    ...
    state->DoString("expCache = {}; exports = setmetatable({}, { __newindex = function(tbl, key, val) print(\"[Swiftly] You can't create a new index on this table.\") end, __index = function(tbl, key) if not expCache[key] then expCache[key] = GetExports(key) end local exp = expCache[key]; return exp; end })");
}