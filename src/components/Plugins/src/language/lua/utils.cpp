...
void SetupLuaUtils(luacpp::LuaState *state, Plugin *plugin)
{
    ...
    if (plugin->HasTimeout || plugin->HasTimers)
        state->DoString("--[[ timers ]] local timerstblsize = nil; local timerst = nil; events:on(\"OnGameTick\", function(simulating, first, last) timerstblsize = #timeoutsTbl; timerst = GetTime(); for i=1,timerstblsize do if timeoutsTbl[i].call - timerst <= 0 then timeoutsTbl[i].cb(); timeoutsRemoveTbl[#timeoutsRemoveTbl + 1] = i end end; for i=1,#timeoutsRemoveTbl do table.remove(timeoutsTbl, timeoutsRemoveTbl[i]) end timeoutsRemoveTbl = {} end)");
}