function LoadHooks(global) {
    ...
    global.AddVHook = (library, vtable_name, offset, argsList, returnType) => {
        if (typeof argsList !== "string") {
            console.log("AddVHook: argsList must be a string.");
            return;
        }
        if (typeof returnType !== "string") {
            console.log("AddVHook: returnType must be a string.");
            return;
        }
    
        let hookId = hooks.AddVirtual(library, vtable_name, offset, argsList, returnType);
    ...
    global.CallHook = (hookHandle, ...args) => {
        ...
        return hooks.Call(hookHandle.hookId, args);
    };
}
